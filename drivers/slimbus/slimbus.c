				...
				enum slim_port_flow flow)
{
	...
	if (la == SLIM_LA_MANAGER)
		...
		pr_err("CODEC connect MC:0x %x, port:%x", mc, pn);/*MM-RC-NickiSS-00960-NickiSS-01138+*/
...
static int disconnect_port_ch(struct slim_controller *ctrl, u32 ph)
{
	...
	pr_err("CODEC disconnect port:%x", pn);/*MM-RC-NickiSS-00960-NickiSS-01138+*/
	ret = slim_processtxn(ctrl, SLIM_MSG_DEST_LOGICALADDR, mc, 0,
...
int slim_reconfigure_now(struct slim_device *sb)
{
	...
	if (!ret && subframe != ctrl->sched.subfrmcode) {
		...
		pr_err("sending subframe:%x,ret:%d\n", wbuf[0], ret);/*MM-RC-NickiSS-00960-NickiSS-01138+*/
	}
	...
	if (!ret) {
		...
		mutex_unlock(&ctrl->m_ctrl);
		mutex_unlock(&ctrl->sched.m_reconf);
		pr_err("slim reconfig done!");/*MM-RC-NickiSS-00960-NickiSS-01138+*/
		return 0;
	}
			...
			enum slim_ch_control chctrl, bool commit)
{
	...
	do {
		...
		pr_err("chan:%d,ctrl:%d,def:%d, ref:%d", slc->chan,
				chctrl, slc->def, slc->ref);/*MM-RC-NickiSS-00960-NickiSS-01138+*/
